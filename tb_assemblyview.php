...
$tb_assembly_view = NULL; // Initialize page object first

class ctb_assembly_view extends ctb_assembly {
	...
	// Page object name
	var $PageObjName = 'tb_assembly_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["assembly_id"] <> "") {
			$this->RecKey["assembly_id"] = $_GET["assembly_id"];
			$KeyUrl .= "&amp;assembly_id=" . urlencode($this->RecKey["assembly_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["assembly_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["assembly_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["assembly_id"] <> "") {
				...
				$this->RecKey["assembly_id"] = $this->assembly_id->QueryStringValue;
			} elseif (@$_POST["assembly_id"] <> "") {
				$this->assembly_id->setFormValue($_POST["assembly_id"]);
				$this->RecKey["assembly_id"] = $this->assembly_id->FormValue;
			} else {
				$sReturnUrl = "tb_assemblylist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "tb_assemblylist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "tb_assemblylist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($tb_assembly_view)) $tb_assembly_view = new ctb_assembly_view();

// Page init
$tb_assembly_view->Page_Init();

// Page main
$tb_assembly_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_assembly_view->Page_Render();
?>
...
var CurrentForm = ftb_assemblyview = new ew_Form("ftb_assemblyview", "view");

// Form_CustomValidate event
ftb_assemblyview.Form_CustomValidate = 
...
ftb_assemblyview.ValidateRequired = true;
<?php } else { ?>
ftb_assemblyview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftb_assemblyview.Lists["x_merk_id"] = {"LinkField":"x_merk_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_merk_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tb_merk"};
ftb_assemblyview.Lists["x_device_id"] = {"LinkField":"x_device_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_device_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tb_device"};
...
<?php if ($tb_assembly->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$tb_assembly_view->IsModal) { ?>
<?php if ($tb_assembly->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $tb_assembly_view->ExportOptions->Render("body") ?>
<?php
	foreach ($tb_assembly_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$tb_assembly_view->IsModal) { ?>
<?php if ($tb_assembly->Export == "") { ?>
...
<?php } ?>
<?php $tb_assembly_view->ShowPageHeader(); ?>
<?php
$tb_assembly_view->ShowMessage();
?>
<form name="ftb_assemblyview" id="ftb_assemblyview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_assembly_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_assembly_view->Token ?>">
<?php } ?>
...
<?php if ($tb_assembly->assembly_id->Visible) { // assembly_id ?>
	<tr id="r_assembly_id">
		<td><span id="elh_tb_assembly_assembly_id"><?php echo $tb_assembly->assembly_id->FldCaption() ?></span></td>
...
<?php echo $tb_assembly->assembly_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_assembly->merk_id->Visible) { // merk_id ?>
	<tr id="r_merk_id">
		<td><span id="elh_tb_assembly_merk_id"><?php echo $tb_assembly->merk_id->FldCaption() ?></span></td>
...
<?php echo $tb_assembly->merk_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_assembly->device_id->Visible) { // device_id ?>
	<tr id="r_device_id">
		<td><span id="elh_tb_assembly_device_id"><?php echo $tb_assembly->device_id->FldCaption() ?></span></td>
...
<?php echo $tb_assembly->device_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_assembly->unit_name->Visible) { // unit_name ?>
	<tr id="r_unit_name">
		<td><span id="elh_tb_assembly_unit_name"><?php echo $tb_assembly->unit_name->FldCaption() ?></span></td>
...
<?php echo $tb_assembly->unit_name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tb_assembly->keterangan->Visible) { // keterangan ?>
	<tr id="r_keterangan">
		<td><span id="elh_tb_assembly_keterangan"><?php echo $tb_assembly->keterangan->FldCaption() ?></span></td>
...
<?php echo $tb_assembly->keterangan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($tb_assembly->Export == "") { ?>
<script type="text/javascript">
ftb_assemblyview.Init();
</script>
<?php } ?>
<?php
$tb_assembly_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($tb_assembly->Export == "") { ?>
...
<?php
$tb_assembly_view->Page_Terminate();
?>
